---
layout: archive
title: "Publications"
permalink: /papers/
author_profile: true
redirect_from:
  - /papers
---


## Publications

For all papers, please see my [Google Scholar](https://scholar.google.com/citations?user=1zmgbZIAAAAJ).

* Probabilistic Soundness Guarantees in LLM Reasoning Chains \[[pdf](https://arxiv.org/abs/2507.12948)\] \[[code](https://github.com/fallcat/ares)\]
  * **Weiqiu You**, Anton Xue, Shreya Havaldar, Delip Rao, Helen Jin, Chris Callison-Burch, Eric Wong
  * EMNLP 2025

* Probabilistic Stability Guarantees for Feature Attributions \[[pdf](https://arxiv.org/abs/2504.13787)\] \[[blog](https://debugml.github.io/soft-stability/)\] \[[code](https://github.com/helenjin/soft_stability/)\]
  * Helen Jin\*, Anton Xue\*, **Weiqiu You**, Surbhi Goel, Eric Wong
  * NeurIPS 2025

* When LRP Diverges from Leave-One-Out in Transformers \[[pdf](https://arxiv.org/abs/2510.18810)\] \[[code](https://github.com/fallcat/attn_loo)\]
  * **Weiqiu You**, Siqi Zeng, Yao-Hung Hubert Tsai, Makoto Yamada, Han Zhao
  * BlackboxNLP @ EMNLP 2025

* Sum-of-Parts: Self-Attributing Neural Networks with End-to-End Learning of Feature Groups \[[pdf](https://arxiv.org/abs/2310.16316)\] \[[blog](https://debugml.github.io/sum-of-parts/)\] \[[code](https://github.com/BrachioLab/sop)\]
  * **Weiqiu You**, Helen Qu, Marco Gatti, Bhuvnesh Jain, Eric Wong
  * ICML 2025
  
* The FIX Benchmark: Extracting Features Interpretable to eXperts \[[pdf](https://arxiv.org/abs/2409.13684)\] \[[blog](https://debugml.github.io/fix/)\] \[[code](https://github.com/BrachioLab/exlib/tree/main/fix)\] \[[website](https://brachiolab.github.io/fix/)\]
  * Helen Jin, Shreya Havaldar, Chaehyeon Kim, Anton Xue, **Weiqiu You**, Helen Qu, Marco Gatti, Daniel A Hashimoto, Bhuvnesh Jain, Amin Madani, Masao Sako, Lyle Ungar, Eric Wong
  * DMLR 2025

* Evaluating Groups of Features via Consistency, Contiguity, and Stability \[[pdf](https://openreview.net/forum?id=IP2etbIEuC)\] 
  * Chaehyeon Kim, **Weiqiu You**, Shreya Havaldar, Eric Wong
  * ICLR 2024 Tiny Paper Track

* A Pretrained Language Model for Cyber Threat Intelligence \[[pdf](https://aclanthology.org/2023.emnlp-industry.12.pdf)\]
  * Youngja Park, **Weiqiu You**
  * EMNLP 2023 Industry Track

* Causal Reasoning of Entities and Events in Procedural Texts \[[pdf](https://aclanthology.org/2023.findings-eacl.31/)\]
  * Li Zhang, Hainiu Xu, Yue Yang, Shuyan Zhou, **Weiqiu You**, Manni Arora, Chris Callison-Burch
  * EACL 2023 Findings

* Macro-Average: Rare Types Are Important Too \[[pdf](https://aclanthology.org/2021.naacl-main.90.pdf)\] \[[code](https://github.com/thammegowda/007-mt-eval-macro)\]
  * Thamme Gowda, **Weiqiu You**, Constantine Lignos, Jonathan May
  * NAACL 2021

* Hard-Coded Gaussian Attention for Neural Machine Translation \[[pdf](https://www.aclweb.org/anthology/2020.acl-main.687.pdf)\] \[[code](https://github.com/fallcat/stupidNMT)\]
  * **Weiqiu You**\*, Simeng Sun\*, Mohit Iyyer
  * ACL 2020

# Preprints

* T-FIX: Text-Based Explanations with Features Interpretable to eXperts \[[pdf](https://shreyahavaldar.com/files/T-FIX.pdf)\] \[[code](https://github.com/BrachioLab/FIX-2)\]
  * Shreya Havaldar\*, Helen Jin\*, Chaehyeon Kim\*, Anton Xue\*, **Weiqiu You**\*, Marco Gatti, Bhuvnesh Jain, Helen Qu, Daniel A Hashimoto, Amin Madani, Rajat Deo, Sameed Ahmed M. Khatana, Gary E. Weissman, Lyle Ungar, Eric Wong
  * Under submission

* NSF-SCIFY: Mining the NSF Awards Database for Scientific Claims \[[pdf](https://arxiv.org/abs/2503.08600)\] 
  * Delip Rao\*, **Weiqiu You**\*, Eric Wong, Chris Callison-Burch
  * Under submission


<!-- {% if author.googlescholar %}
  You can also find my articles on <u><a href="{{author.googlescholar}}">my Google Scholar profile</a>.</u>
{% endif %}

{% include base_path %}

{% for post in site.publications reversed %}
  {% include archive-single.html %}
{% endfor %} -->
